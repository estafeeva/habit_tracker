...
def validate_reward_or_pleasant_habit(is_pleasant_habit, linked_habit, reward):
    ...
    """
    if is_pleasant_habit is False and reward and linked_habit:
        raise serializers.ValidationError(
            "Вы не можете заполнить одновременно и связанную "
            "привычку, и вознаграждение. Выберите что-то одно."
        )

    if not is_pleasant_habit and not reward and not linked_habit:
        raise serializers.ValidationError(
            "Заполните или связанную привычку, или вознаграждение."
        )
    ...
    """
    if (
        not is_pleasant_habit
        and linked_habit
        and not linked_habit.is_pleasant_habit
    ):
        raise serializers.ValidationError(
